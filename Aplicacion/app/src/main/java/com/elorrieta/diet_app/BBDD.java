...
public class BBDD extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("create table tiene(numComensales integer, nomIngrediente varchar(10), id integer, cantidad integer, unidad varchar(10), foreign key(numComensales,id) references nComensales(numComensales,id), foreign key(nomIngrediente) references ingrediente(nomIngrediente), primary key(nomIngrediente, numComensales, id))");
        db.execSQL("create table ingrediente(nomIngrediente varchar(10) primary key, foto varchar(50), precio float)");
        db.execSQL("create table hay(nomAlmacen varchar(20), nomIngrediente varchar(10), unidad varchar(10), cantidad integer, foreign key(nomIngrediente) references ingrediente(nomIngrediente), foreign key(nomAlmacen) references almacen(nomAlmacen), primary key(nomAlmacen, nomIngrediente))");
